...
#define LRYScreenpW(ww)  (float)ww*([[UIScreen mainScreen] bounds].size.width)/750
#define LRYScreenpH(hh)  (float)hh*([[UIScreen mainScreen] bounds].size.height)/1334
...
#define LRYsystemFontOfSize(value) (float)((value)/414.0f*[UIScreen mainScreen].bounds.size.width)
...
#define LRYWidthCoefficient (ScreenHeight == 812.0 ? 375.0/375.0 : ScreenWidth/375.0) //适配系数
#define LRYHeightCoefficient (ScreenHeight == 812.0 ? 667.0/667.0 : ScreenHeight/667.0) //适配系数
...
#define MainColor    LRYColor(0, 174, 255)
...
#define LRYColor(r, g, b) [UIColor colorWithRed:(r)/255.0 green:(g)/255.0 blue:(b)/255.0 alpha:1.0]
#define LRYCOLOR(rgbValue)    [UIColor colorWithRed:((float)((rgbValue & 0xFF0000) >> 16))/255.0 green:((float)((rgbValue & 0xFF00) >> 8))/255.0 blue:((float)(rgbValue & 0xFF))/255.0 alpha:1.0]
